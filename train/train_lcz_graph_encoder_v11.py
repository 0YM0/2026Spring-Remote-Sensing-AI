#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V11

V11 proposal: center-anchor + core/ring contrast graph + residual-gated RS fusion.

Motivation
----------
V7 improves most classes but can confuse LCZ1/5/6 when the morphology of the
target cell is diluted by a broad 500 m graph context. V11 keeps V7's graph
context and training protocol while making the target-cell neighborhood
explicit:
  1) a 150 m center morphology anchor token
  2) separate core/ring/contrast graph pooling
  3) RS-first residual-gated graph correction

No class-specific loss, sampling, or decision rule is used.

Recommended first run:
CUDA_VISIBLE_DEVICES=0 python train/train_lcz_graph_encoder_v11.py \
  ...
  --exp_name graph_v11_core_ring_gated_oa \
  --patch_size 33 --graph_context_m 500 --core_context_m 150 --edge_mode hybrid --knn 8 --radius_m 120 \
  ...
  --class_weight --monitor oa --rebuild_graph_cache
"""
...
# V11 node features (same dimensionality as V7):
...
# 17 is_building, 18 is_ump_token, 19 scale_norm, 20 is_center_anchor, 21 has_node
NODE_DIM = 22
...
# 0 none/padding, 1 self, 2 knn, 3 radius, 4 knn+radius, 5 building-UMP,
# 6 token-token, 7 building-center-anchor
NUM_EDGE_TYPES = 8
...
class BuildingGraphProviderV11:
    """
    V11 graph provider: V7 graph plus a center morphology anchor token.
    """
    def __init__(self,
                 shp_path: Path,
                 gt_path: Path,
        ...
        self.core_context_m = float(core_context_m)
        if not 0 < self.core_context_m <= self.context_m:
            raise ValueError(f"core_context_m must be in (0, graph_context_m]. Got {self.core_context_m}")
        self.core_half_m = self.core_context_m / 2.0
        ...
        if len(self.global_scales_m) != len(DEFAULT_GLOBAL_SCALES_M):
            raise ValueError(f"V11 expects exactly {len(DEFAULT_GLOBAL_SCALES_M)} global scales. Got {self.global_scales_m}")
        ...
        print(f"Context={self.context_m:.1f}m, core_context={self.core_context_m:.1f}m, global_scales={self.global_scales_m}, edge_mode={edge_mode}, knn={knn}, radius={radius_m}, max_nodes={max_nodes}")
    ...
    def get_graph(self, row50: int, col50: int):
        ...
        if len(idx) == 0:
            ...
            center_anchor = self.make_center_anchor_node(idx_all, dx_all, dy_all, dist_all)
            ump_nodes, _ = self.make_ump_token_nodes(idx_all, dx_all, dy_all, dist_all)
            if len(ump_nodes) > 0 and np.any(ump_nodes[:, -1] > 0):
                node = np.concatenate([center_anchor[None, :], ump_nodes], axis=0).astype(np.float32)
                empty_xy = np.zeros((0, 2), dtype=np.float32)
                empty_coord = np.zeros((0,), dtype=np.float32)
                empty_core = np.zeros((0,), dtype=bool)
                empty_ump_masks = [np.zeros((0,), dtype=bool) for _ in self.global_scales_m]
                adj, etype = self.make_adj_and_edge_type_with_tokens(
                    empty_xy, empty_coord, empty_coord, empty_core, empty_ump_masks
                )
                n = node.shape[0]
        ...
        core_building_mask = (np.abs(dx) <= self.core_half_m) & (np.abs(dy) <= self.core_half_m)
        node = np.concatenate([building_node, center_anchor[None, :], ump_nodes], axis=0).astype(np.float32)
        adj, etype = self.make_adj_and_edge_type_with_tokens(
            xy_rel, dx, dy, core_building_mask, ump_scale_masks
        )
    ...
    def make_adj_and_edge_type_with_tokens(self,
                                           xy_rel: np.ndarray,
                                           dx: np.ndarray,
                                           dy: np.ndarray,
                                           core_building_mask: np.ndarray,
                                           ump_scale_masks: list):
        nb = xy_rel.shape[0]
        ...
        # The center anchor communicates the target-cell neighborhood directly.
        adj[center_i, center_i] = 1.0
        etype[center_i, center_i] = 1
        if nb > 0:
            dist = np.sqrt(dx * dx + dy * dy)
            weights = np.exp(-dist / max(self.core_half_m, 1.0)).astype(np.float32)
            weights = np.maximum(weights, 0.35) * core_building_mask.astype(np.float32)
            for bi in np.where(core_building_mask)[0]:
                adj[center_i, bi] = adj[bi, center_i] = max(float(weights[bi]), 0.35)
                etype[center_i, bi] = etype[bi, center_i] = 7
        ...
        # Anchor-to-token links let local morphology interact with broader scales.
        for u in range(nu):
            ui = ump_start + u
            adj[center_i, ui] = adj[ui, center_i] = 0.5
            etype[center_i, ui] = etype[ui, center_i] = 6
        np.fill_diagonal(adj, 1.0)
        np.fill_diagonal(etype, 1)
        return adj, etype

    def make_center_anchor_node(self, idx_all, dx_all, dy_all, dist_all):
        inside = (np.abs(dx_all) <= self.core_half_m) & (np.abs(dy_all) <= self.core_half_m)
        idx = idx_all[inside]
        dist = dist_all[inside]
        feat = np.zeros((NODE_DIM,), dtype=np.float32)
        if len(idx) > 0:
            scale_feat = self.make_global_features_for_scale(
                self.area[idx], self.story[idx], dist, self.core_context_m, perim=self.perimeter[idx]
            )
            feat[0] = scale_feat[3]   # core built ratio
            feat[1] = scale_feat[6]   # core mean story
            feat[2] = 1.0 - scale_feat[11]
            feat[5] = float(np.clip(self.core_context_m / max(self.context_m, 1.0), 0.0, 1.0))
            feat[8] = scale_feat[14]
            feat[9] = scale_feat[15]
            feat[10] = scale_feat[16]
            feat[11] = scale_feat[1]  # core density
            feat[12] = scale_feat[3]
            feat[13] = scale_feat[0]  # core count
            feat[14] = scale_feat[8]
            feat[15] = scale_feat[12]
            feat[16] = scale_feat[4]
        feat[19] = float(np.clip(self.core_context_m / max(self.context_m, 1.0), 0.0, 1.0))
        feat[20] = 1.0
        feat[21] = 1.0
        return feat
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: BuildingGraphProviderV11, cache_path: Path, split_name: str):
    ...
    payload = {
        ...
        "meta": {
            ...
            "core_context_m": float(graph_provider.core_context_m),
...
class CachedLCZGraphDatasetV11(Dataset):
...
class GraphAttentionBlockV11(nn.Module):
...
class BuildingGraphEncoderV11(nn.Module):
    def __init__(self,
                 in_dim: int = NODE_DIM,
        ...
        self.layers = nn.ModuleList([GraphAttentionBlockV11(hidden_dim, heads=heads, dropout=dropout) for _ in range(num_layers)])
        ...
        self.global_contrast_mlp = nn.Sequential(
            nn.Linear(GLOBAL_BASE_DIM * 2, hidden_dim), nn.GELU(), nn.LayerNorm(hidden_dim)
        )
        pooled_dim = hidden_dim * 7
    ...
    @staticmethod
    def masked_mean(h, region_mask):
        denom = region_mask.sum(dim=1, keepdim=True).clamp_min(1.0)
        return (h * region_mask.unsqueeze(-1)).sum(dim=1) / denom
    ...
    def forward(self, node, adj, edge_type, mask, glob):
        ...
        building_mask = mask * (node[:, :, 17] > 0.5).float()
        core_mask = building_mask * (
            (node[:, :, 3].abs() <= self.core_ratio)
            & (node[:, :, 4].abs() <= self.core_ratio)
        ).float()
        ring_mask = building_mask * (1.0 - (core_mask > 0).float())
        anchor_mask = mask * (node[:, :, 20] > 0.5).float()

        overall_attn = self.attn_pool(h, mask)
        building_mean = self.masked_mean(h, building_mask)
        core_mean = self.masked_mean(h, core_mask)
        ring_mean = self.masked_mean(h, ring_mask)
        core_ring_contrast = core_mean - ring_mean
        anchor_feat = self.masked_mean(h, anchor_mask)
        global_feat = self.global_mlp(glob)

        scale0 = glob[:, :GLOBAL_BASE_DIM]
        scale1 = glob[:, GLOBAL_BASE_DIM:GLOBAL_BASE_DIM * 2]
        scale2 = glob[:, GLOBAL_BASE_DIM * 2:GLOBAL_BASE_DIM * 3]
        global_contrast = self.global_contrast_mlp(torch.cat([scale0 - scale1, scale0 - scale2], dim=1))
        ...
        return self.out_mlp(torch.cat([
            ...
            core_ring_contrast,
            anchor_feat,
            global_feat + global_contrast,
        ], dim=1))


class RSGraphLCZModelV11(nn.Module):
    def __init__(self,
                 rs_channels: int,
                 num_classes: int,
                 patch_size: int = 33,
        ...
        self.graph_encoder = BuildingGraphEncoderV11(
            in_dim=NODE_DIM,
            ...
            heads=graph_heads,
            dropout=dropout * 0.4,
            core_ratio=float(core_context_m) / max(float(graph_context_m), 1.0),
        )
        self.rs_head = nn.Sequential(
            nn.LayerNorm(256),
            nn.Dropout(dropout),
            nn.Linear(256, 192), nn.GELU(),
        ...
        self.correction_gate = nn.Sequential(
            nn.LayerNorm(256 + graph_out),
            nn.Linear(256 + graph_out, num_classes),
        )
        nn.init.constant_(self.correction_gate[-1].bias, float(gate_init_bias))
        nn.init.normal_(self.graph_correction[-1].weight, mean=0.0, std=1e-3)
        nn.init.zeros_(self.graph_correction[-1].bias)
    ...
    def forward(self, rs, node, adj, edge_type, mask, glob):
        ...
        joint = torch.cat([rs_feat, graph_feat], dim=1)
        rs_logits = self.rs_head(rs_feat)
        correction = self.graph_correction(joint)
        gate = torch.sigmoid(self.correction_gate(joint))
        return rs_logits + gate * correction
...
def train(args):
    ...
    tag = f"v11_ctx{int(args.graph_context_m if args.graph_context_m else args.patch_size*10)}_core{int(args.core_context_m)}_gs{'-'.join(str(int(x)) for x in args.global_scales_m)}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}"
    cache_paths = {
    ...
    train_ds = CachedLCZGraphDatasetV11(rs_stack, cache_paths["train"], args.patch_size)
    val_ds = CachedLCZGraphDatasetV11(rs_stack, cache_paths["val"], args.patch_size)
    test_ds = CachedLCZGraphDatasetV11(rs_stack, cache_paths["test"], args.patch_size)
    print(f"Dataset: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v11)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v11)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v11)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    model = RSGraphLCZModelV11(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if metric > best_metric:
            ...
            torch.save({
                "epoch": epoch,
                "model_version": "v11_core_ring_gated",
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V11",
        "model_version": "v11_core_ring_gated",
        ...
        "core_context_m": float(args.core_context_m),
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v11")
    p.add_argument("--patch_size", type=int, default=33)
    p.add_argument("--graph_context_m", type=float, default=500.0)
    p.add_argument("--core_context_m", type=float, default=150.0)
