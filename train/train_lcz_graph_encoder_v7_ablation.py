#!/usr/bin/env python3
"""
Controlled ablation study for CNN_RS_BuildingGraphEncoder_V7.

The original V7 file is not modified. This script keeps V7 training settings
fixed and removes one graph component at a time:

  full                  Complete V7 reference model
  no_salient_selection  Replace salient node sampling with nearest-node sampling
  no_ump_tokens         Remove multi-scale UMP morphology token nodes
  no_global_context     Remove the multi-scale global morphology feature branch
  no_edge_type          Remove learned edge-type attention bias

Recommended experiments use the same split, seed, loss, monitor, and optimizer.
Only no_salient_selection requires a different graph cache. Other variants can
reuse the full V7 cache.
...
class BuildingGraphProviderV7:
    ...
    def __init__(self,
                 ...
                 num_sectors: int = 8,
                 use_salient_selection: bool = True):
        ...
        self.use_salient_selection = bool(use_salient_selection)
        ...
        print(f"Salient selection={self.use_salient_selection}, ratios: near={self.salient_near_ratio}, large={self.salient_large_ratio}, high={self.salient_high_ratio}, sectors={self.num_sectors}")
    ...
    def select_salient_nodes(self, idx, dx, dy, dist):
        ...
        if not self.use_salient_selection:
            return np.argsort(dist)[:self.max_nodes].astype(np.int64)
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: BuildingGraphProviderV7, cache_path: Path, split_name: str):
    ...
    payload = {
        ...
        "meta": {
            ...
            "use_salient_selection": bool(graph_provider.use_salient_selection),
...
class CachedLCZGraphDatasetV7(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        glob = self.globals[idx].float()

        if not self.use_ump_tokens:
            keep = node[:, 18] < 0.5
            if bool(keep.any()):
                node = node[keep]
                adj = adj[keep][:, keep]
                edge_type = edge_type[keep][:, keep]
                mask = mask[keep]
            else:
                node = torch.zeros((1, NODE_DIM), dtype=torch.float32)
                adj = torch.ones((1, 1), dtype=torch.float32)
                edge_type = torch.ones((1, 1), dtype=torch.long)
...
class BuildingGraphEncoderV7(nn.Module):
    ...
    def forward(self, node, adj, edge_type, mask, glob):
        ...
        global_feat = self.global_mlp(glob) if self.use_global_context else torch.zeros_like(mean_pool)
...
def train(args):
    set_seed(args.seed)
    use_salient_selection = args.ablation != "no_salient_selection"
    use_ump_tokens = args.ablation != "no_ump_tokens"
    use_global_context = args.ablation != "no_global_context"
    use_edge_type = args.ablation != "no_edge_type"
    ablation_config = {
        "ablation": args.ablation,
        "use_salient_selection": use_salient_selection,
        "use_ump_tokens": use_ump_tokens,
        "use_global_context": use_global_context,
        "use_edge_type": use_edge_type,
    }
    print("Ablation configuration:", json.dumps(ablation_config, indent=2))
    ...
    sampling_suffix = "" if use_salient_selection else "_nearest"
    tag = f"v7_ctx{int(args.graph_context_m if args.graph_context_m else args.patch_size*10)}_gs{'-'.join(str(int(x)) for x in args.global_scales_m)}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}{sampling_suffix}"
    cache_paths = {
    ...
    train_ds = CachedLCZGraphDatasetV7(
        rs_stack, cache_paths["train"], args.patch_size,
        use_ump_tokens=use_ump_tokens,
        use_global_context=use_global_context,
    )
    val_ds = CachedLCZGraphDatasetV7(
        rs_stack, cache_paths["val"], args.patch_size,
        use_ump_tokens=use_ump_tokens,
        use_global_context=use_global_context,
    )
    test_ds = CachedLCZGraphDatasetV7(
        rs_stack, cache_paths["test"], args.patch_size,
        use_ump_tokens=use_ump_tokens,
        use_global_context=use_global_context,
    )
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V7_Ablation",
        **ablation_config,
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="graph_v7_ablation")
    p.add_argument(
        "--ablation",
        type=str,
        default="full",
        choices=["full", "no_salient_selection", "no_ump_tokens", "no_global_context", "no_edge_type"],
        help="Remove exactly one V7 component while keeping all training settings fixed.",
    )
