#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V2

Changes vs v1:
- Precomputed graph cache is kept.
- Graph context can be larger than the RS patch via --graph_context_m.
- Edge mode supports knn / radius / hybrid.
- Node features are expanded with shape compactness, bbox fill ratio, angle cos/sin, missing-story flag.
- Graph-level morphology features are added: building count, density, total area, built ratio, story stats, area stats, distance stats.
- Graph branch uses masked graph attention + residual FFN + attention/mean/max pooling.
- Validation checkpoint can monitor OA or Macro-F1.

Recommended first run:
CUDA_VISIBLE_DEVICES=2 python train/train_lcz_graph_encoder_v2.py \
...
CUDA_VISIBLE_DEVICES=2 python train/train_lcz_graph_encoder_v2.py \
  ...
  --class_weight \
  --monitor oa \
  --exp_name cnn_rs_building_graph_encoder_v2_hybrid500_oa
  
  --rebuild_graph_cache

#  --rebuild_graph_cache(최초 실행만 적용)
# 다시 돌릴떄는 --rebuild_graph_cache 제거
"""
...
# 11 rect_fill, 12 compactness, 13 has_building
NODE_DIM = 14
...
class BuildingGraphProviderV2:
    def __init__(self,
        ...
        self.count_scale = max(float(np.percentile([10, 20, 40, 80, max_nodes], 80)), 1.0)
        print(f"Context={self.context_m:.1f}m, edge_mode={edge_mode}, knn={knn}, radius={radius_m}, max_nodes={max_nodes}")
    ...
    def get_graph(self, row50: int, col50: int):
        ...
        if len(idx) > self.max_nodes:
            # Prefer closer and larger buildings, not only closest.
            score = dist / max(self.half_size_m, 1e-6) - 0.15 * np.clip(np.log1p(self.area[idx]) / self.area_scale, 0, 1)
            order = np.argsort(score)[:self.max_nodes]
        ...
        node = np.stack([
            ...
            rect_fill, compactness, has_building,
        ], axis=1).astype(np.float32)

        adj = self.make_adj(np.stack([dx, dy], axis=1).astype(np.float32))
        mask = np.ones((node.shape[0],), dtype=np.float32)
        glob = self.make_global_features(area, story, dist)
        return node, adj, mask, glob

    def make_adj(self, xy_rel: np.ndarray):
        n = xy_rel.shape[0]
    ...
    def make_global_features(self, area: np.ndarray, story: np.ndarray, dist: np.ndarray):
        ...
        density_norm = np.clip((n / max(self.context_area_m2, 1.0)) / 0.001, 0.0, 1.0)  # 0.001 buildings/m2 ~= 1000/km2
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: BuildingGraphProviderV2, cache_path: Path, split_name: str):
    ...
    for i, (r50, c50) in enumerate(zip(rows, cols), start=1):
        node, adj, mask, glob = graph_provider.get_graph(int(r50), int(c50))
...
class CachedLCZGraphDatasetV2(Dataset):
    def __init__(self, rs_stack: np.ndarray, cache_pt: Path, patch_size: int = 33):
        ...
        if not (len(self.rows50) == len(self.cols50) == len(self.labels) == len(self.nodes) == len(self.adjs) == len(self.masks) == len(self.globals)):
...
def graph_collate_v2(batch):
    ...
    for i, b in enumerate(batch):
        ...
        adjs[i, :n, :n] = b["adj"]
        masks[i, :n] = b["mask"]
    return rs, nodes, adjs, masks, glob, y
...
class GraphAttentionBlock(nn.Module):
    ...
    def forward(self, h, adj, mask):
        ...
        # adjacency as soft bias. Non-edge becomes very negative.
...
class BuildingGraphEncoderV2(nn.Module):
    def __init__(self, in_dim: int = NODE_DIM, global_dim: int = GLOBAL_DIM, hidden_dim: int = 96, out_dim: int = 192, num_layers: int = 3, heads: int = 4, dropout: float = 0.1):
        ...
        self.layers = nn.ModuleList([GraphAttentionBlock(hidden_dim, heads=heads, dropout=dropout) for _ in range(num_layers)])
        self.attn_pool = AttentionPool(hidden_dim)
        self.global_mlp = nn.Sequential(nn.Linear(global_dim, hidden_dim // 2), nn.GELU(), nn.LayerNorm(hidden_dim // 2))
        pooled_dim = hidden_dim * 3 + hidden_dim // 2
        self.out_mlp = nn.Sequential(
            nn.Linear(pooled_dim, out_dim), nn.GELU(), nn.LayerNorm(out_dim), nn.Dropout(dropout),
            nn.Linear(out_dim, out_dim), nn.GELU(),
        )

    def forward(self, node, adj, mask, glob):
...
class RSGraphLCZModelV2(nn.Module):
    def __init__(self, rs_channels: int, num_classes: int, patch_size: int = 33, graph_hidden: int = 96, graph_out: int = 192, graph_layers: int = 3, graph_heads: int = 4, dropout: float = 0.35):
        ...
        self.graph_encoder = BuildingGraphEncoderV2(in_dim=NODE_DIM, global_dim=GLOBAL_DIM, hidden_dim=graph_hidden, out_dim=graph_out, num_layers=graph_layers, heads=graph_heads, dropout=dropout * 0.4)
    ...
    def forward(self, rs, node, adj, mask, glob):
        rs_feat = self.rs_encoder(rs)
        graph_feat = self.graph_encoder(node, adj, mask, glob)
...
def evaluate(model, loader, device, num_classes):
    ...
    total_loss, n = 0.0, 0
    with torch.no_grad():
        for rs, node, adj, mask, glob, y in loader:
            ...
            logits = model(rs, node, adj, mask, glob)
...
def train(args):
    ...
    tag = f"ctx{int(args.graph_context_m if args.graph_context_m else args.patch_size*10)}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}"
    cache_paths = {
    ...
    if need_cache:
        ...
        graph_provider = BuildingGraphProviderV2(
    ...
    train_ds = CachedLCZGraphDatasetV2(rs_stack, cache_paths["train"], args.patch_size)
    val_ds = CachedLCZGraphDatasetV2(rs_stack, cache_paths["val"], args.patch_size)
    test_ds = CachedLCZGraphDatasetV2(rs_stack, cache_paths["test"], args.patch_size)
    print(f"Dataset: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v2)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v2)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v2)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    model = RSGraphLCZModelV2(
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        total_loss, n = 0.0, 0
        for rs, node, adj, mask, glob, y in train_loader:
            rs = rs.to(device, non_blocking=True)
            node = node.to(device, non_blocking=True)
            adj = adj.to(device, non_blocking=True)
            mask = mask.to(device, non_blocking=True)
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V2",
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v2")
    p.add_argument("--patch_size", type=int, default=33)
    p.add_argument("--graph_context_m", type=float, default=None)
    ...
    p.add_argument("--monitor", type=str, default="macro_f1", choices=["oa", "macro_f1"])
