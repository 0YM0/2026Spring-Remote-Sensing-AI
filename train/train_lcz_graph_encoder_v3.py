#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V3
...
2) Keep building polygons as object nodes.
3) Use typed spatial edges: self, center-building, building-kNN, building-radius, hybrid.
4) Use edge-aware graph attention.
5) Use center-aware graph readout and RS-guided gated fusion.
...
CUDA_VISIBLE_DEVICES=2 python train/train_lcz_graph_encoder_v3.py \
  ...
  --exp_name graph_v3_center_hybrid500 \
  --patch_size 33 \
  --graph_context_m 500 \
  --edge_mode hybrid \
  --knn 6 \
...
# Node features:
# 0 is_center, 1 is_building,
# 2 area_norm, 3 story_norm, 4 story_missing,
# 5 rel_x, 6 rel_y, 7 dist_norm, 8 cos_theta, 9 sin_theta,
# 10 bbox_w_norm, 11 bbox_h_norm, 12 aspect_norm,
# 13 rect_fill, 14 compactness
NODE_DIM = 15
...
class CenterAwareBuildingGraphProvider:
    def __init__(self,
                 shp_path: Path,
                 gt_path: Path,
                 story_col: str = "A10",
        ...
        self.max_building_nodes = max(1, self.max_nodes - 1)  # reserve node 0 for center node
        self.knn = int(knn)
    ...
    def empty_graph(self):
        ...
        node[0, 0] = 1.0  # is_center
        adj = np.ones((1, 1), dtype=np.float32)
        edge_type = np.ones((1, 1), dtype=np.int64) * EDGE_SELF
    ...
    def get_graph(self, row50: int, col50: int):
        ...
        if len(idx) > self.max_building_nodes:
            # Prefer buildings that are closer, larger, and taller.
        ...
        bnode = self.make_building_node(area, story, bbox_w, bbox_h, perim, dx, dy, dist)
        center_node = np.zeros((1, NODE_DIM), dtype=np.float32)
        center_node[0, 0] = 1.0  # is_center
        node = np.concatenate([center_node, bnode], axis=0)
        adj, edge_type = self.make_adj_with_center(np.stack([dx, dy], axis=1).astype(np.float32), dist.astype(np.float32))
    ...
    def make_adj_with_center(self, xy_rel: np.ndarray, dist_center: np.ndarray):
        nb = xy_rel.shape[0]
        n = nb + 1
        ...
        # Center-building edges: complete, distance-weighted.
        center_sigma = max(self.half_size_m, 1e-6)
        cw = np.exp(-dist_center / center_sigma).astype(np.float32)
        adj[0, 1:] = cw
        adj[1:, 0] = cw
        edge_type[0, 1:] = EDGE_CENTER
        edge_type[1:, 0] = EDGE_CENTER
        ...
        adj[1:, 1:] = np.maximum(adj[1:, 1:], b_adj)
        edge_type[1:, 1:] = np.maximum(edge_type[1:, 1:], b_type)
        np.fill_diagonal(adj, 1.0)
        np.fill_diagonal(edge_type, EDGE_SELF)
        return adj, edge_type
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: CenterAwareBuildingGraphProvider, cache_path: Path, split_name: str):
...
class RSGraphLCZModelV3(nn.Module):
...
def train(args):
    ...
    tag = f"v3_ctx{context_tag}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}"
    cache_paths = {
    ...
    if need_cache:
        ...
        graph_provider = CenterAwareBuildingGraphProvider(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val["oa"] > best_val_oa:
            ...
            torch.save({
                ...
                "graph_layers": args.graph_layers,
                "graph_heads": args.graph_heads,
                "model_version": "v3_center_aware_gated_fusion",
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V3_CenterAwareGatedFusion",
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v3")
