#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V9

V9 proposal: V7 salient-node + multi-scale morphology token graph, with GLCFormer-lite
cross-attention fusion between CNN spatial tokens and building graph tokens.
Graph construction/cache is compatible with V7; the main change is replacing late concat
fusion with graph-conditioned image feature-map fusion.

Recommended first run:
CUDA_VISIBLE_DEVICES=0 python train/train_lcz_graph_encoder_v9.py \
  ...
  --exp_name cnn_rs_building_graph_encoder_v9 \
...
class ConvFeatureEncoderV9(nn.Module):
    """
    CNN branch for V9.

    Difference from V7 ConvEncoder:
    - V7 flattened the CNN feature map and produced a single 256-dim vector.
    - V9 keeps the final CNN feature map [B, C, H, W] so graph tokens can be fused
      into spatial image tokens before classification.
    """
    def __init__(self, in_channels: int, patch_size: int, base_channels: int = 32, dropout: float = 0.0):
        super().__init__()
        ...
        self.feat_hw = (int(feat.shape[2]), int(feat.shape[3]))
...
class BuildingGraphEncoderV9(nn.Module):
    """
    V7 graph encoder kept, but it now exposes node-level graph tokens for
    image-graph cross-attention fusion.

    Outputs:
    - graph_tokens: [B, N, hidden_dim], node/morphology-token embeddings after edge-aware GAT
    - graph_summary: [B, out_dim], mean/max/attention/global pooled graph descriptor
    """
    ...
    def encode_tokens(self, node, adj, edge_type, mask):
        h = self.input_mlp(node) * mask.unsqueeze(-1)
        for layer in self.layers:
            h = layer(h, adj, edge_type, mask)
        return h

    def pool_summary(self, h, mask, glob):
    ...
    def forward(self, node, adj, edge_type, mask, glob):
        h = self.encode_tokens(node, adj, edge_type, mask)
        summary = self.pool_summary(h, mask, glob)
        return h, summary


class CrossAttentionFusionBlockV9(nn.Module):
    """
    GLCFormer-lite fusion block.

    Image tokens query graph tokens. The residual update is warm-started with a
    small learnable scale, following the same stability intuition as GLCFormer:
    start close to RGB/CNN-only behavior, then learn how much graph context to inject.
    """
    def __init__(self, dim: int = 128, heads: int = 4, dropout: float = 0.1, residual_init: float = 0.05):
        super().__init__()
        if dim % heads != 0:
            raise ValueError(f"fusion dim {dim} must be divisible by fusion heads {heads}")
    ...
    def forward(self, img_tokens, graph_tokens, graph_mask):
        key_padding_mask = graph_mask <= 0
        # If a batch item has all graph tokens masked, MultiheadAttention would produce NaNs.
        # V7/V9 graph cache normally keeps at least one valid token; this guard is for safety.
        ...
        kv = self.graph_norm(graph_tokens)
        delta, _ = self.cross_attn(q, kv, kv, key_padding_mask=key_padding_mask, need_weights=False)
        img_tokens = img_tokens + self.residual_scale * self.dropout(delta)
        img_tokens = img_tokens + self.ffn(self.ffn_norm(img_tokens))
        return img_tokens


class ImageGraphFusionV9(nn.Module):
    """
    Spatial graph-image fusion for patch-level LCZ classification.

    1) CNN feature map [B, C, H, W] is flattened into image tokens.
    2) V7 graph tokens are projected into the same fusion dimension.
    3) Image tokens attend to graph tokens through one or more cross-attention blocks.
    4) Fused image tokens are pooled for classification.
    """
    def __init__(self, img_channels: int, graph_hidden: int, graph_summary_dim: int, feat_hw: tuple, fusion_dim: int = 128, heads: int = 4, num_layers: int = 2, dropout: float = 0.1, residual_init: float = 0.05):
        super().__init__()
        self.fusion_dim = int(fusion_dim)
        self.feat_hw = tuple(feat_hw)
        num_img_tokens = int(feat_hw[0] * feat_hw[1])
        self.img_proj = nn.Linear(img_channels, fusion_dim)
        self.graph_proj = nn.Linear(graph_hidden, fusion_dim)
        self.graph_gate = nn.Sequential(nn.Linear(graph_summary_dim, fusion_dim), nn.Sigmoid())
        self.img_pos = nn.Parameter(torch.zeros(1, num_img_tokens, fusion_dim))
        nn.init.trunc_normal_(self.img_pos, std=0.02)
        self.blocks = nn.ModuleList([
            CrossAttentionFusionBlockV9(fusion_dim, heads=heads, dropout=dropout, residual_init=residual_init)
            for _ in range(num_layers)
        ])
        self.attn_pool = AttentionPool(fusion_dim)
        self.out_norm = nn.LayerNorm(fusion_dim * 3 + graph_summary_dim)

    def forward(self, img_feat, graph_tokens, graph_mask, graph_summary):
        B, C, H, W = img_feat.shape
        img_tokens = img_feat.flatten(2).transpose(1, 2)  # [B, HW, C]
        img_tokens = self.img_proj(img_tokens)
        if img_tokens.shape[1] == self.img_pos.shape[1]:
            img_tokens = img_tokens + self.img_pos

        graph_tokens = self.graph_proj(graph_tokens)
        gate = self.graph_gate(graph_summary).unsqueeze(1)
        graph_tokens = graph_tokens * gate

        for block in self.blocks:
            img_tokens = block(img_tokens, graph_tokens, graph_mask)

        img_mask = torch.ones((B, img_tokens.shape[1]), dtype=img_tokens.dtype, device=img_tokens.device)
        mean_pool = img_tokens.mean(dim=1)
        max_pool = img_tokens.max(dim=1).values
        attn_pool = self.attn_pool(img_tokens, img_mask)
        return self.out_norm(torch.cat([mean_pool, max_pool, attn_pool, graph_summary], dim=1))


class RSGraphLCZModelV9(nn.Module):
    def __init__(self,
                 rs_channels: int,
                 num_classes: int,
                 patch_size: int = 33,
                 ...
                 fusion_heads: int = 4,
                 fusion_layers: int = 2,
                 fusion_residual_init: float = 0.05,
                 dropout: float = 0.35):
        super().__init__()
        self.rs_encoder = ConvFeatureEncoderV9(rs_channels, patch_size, base_channels=32, dropout=dropout * 0.15)
        self.graph_encoder = BuildingGraphEncoderV9(
            in_dim=NODE_DIM,
        ...
        self.fusion = ImageGraphFusionV9(
            img_channels=self.rs_encoder.out_channels,
            graph_hidden=graph_hidden,
            graph_summary_dim=graph_out,
            feat_hw=self.rs_encoder.feat_hw,
        ...
        classifier_in = fusion_dim * 3 + graph_out
        self.classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(classifier_in, 384), nn.GELU(),
    ...
    def forward(self, rs, node, adj, edge_type, mask, glob):
        img_feat = self.rs_encoder(rs)
        graph_tokens, graph_summary = self.graph_encoder(node, adj, edge_type, mask, glob)
        fused_feat = self.fusion(img_feat, graph_tokens, mask, graph_summary)
...
def train(args):
    ...
    model = RSGraphLCZModelV9(
        ...
        fusion_dim=args.fusion_dim, fusion_heads=args.fusion_heads,
        fusion_layers=args.fusion_layers, fusion_residual_init=args.fusion_residual_init,
        dropout=args.dropout,
    ).to(device)
    num_params = count_parameters(model)
    print("Device:", device)
    print("Trainable parameters:", f"{num_params:,}")
    print(f"V9 fusion: dim={args.fusion_dim}, heads={args.fusion_heads}, layers={args.fusion_layers}, residual_init={args.fusion_residual_init}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if metric > best_metric:
            ...
            torch.save({
                ...
                "fusion_residual_init": args.fusion_residual_init,
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V9",
        ...
        "fusion_residual_init": float(args.fusion_residual_init),
        "loss": args.loss,
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v9")
    ...
    p.add_argument("--fusion_residual_init", type=float, default=0.05)
