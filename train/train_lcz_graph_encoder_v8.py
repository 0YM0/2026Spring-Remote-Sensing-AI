#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V8

V7 proposal: V6 backbone + morphology-aware node features + distance-binned edge types + scale-attentive global context + interaction fusion.
...
CUDA_VISIBLE_DEVICES=2 python train/train_lcz_graph_encoder_v8.py \
  ...
  --exp_name graph_v7_salient_multiscale_edgeaware \
  --patch_size 33 \
  --graph_context_m 500 \
  ...
  --class_weight \
  --monitor oa \
  --exp_name graph_v7_salient_multiscale_edgeaware_oa
...
# V7 node features:
# 0 area_norm, 1 story_norm, 2 story_missing,
# 3 rel_x, 4 rel_y, 5 dist_norm, 6 cos_theta, 7 sin_theta,
# 8 bbox_w_norm, 9 bbox_h_norm, 10 aspect_norm,
# 11 rect_fill, 12 compactness,
# 13 local_count_density_80m, 14 local_area_density_80m,
# 15 local_story_80m, 16 nearest_neighbor_dist_norm,
# 17 salient_score, 18 has_building
NODE_DIM = 19
...
# 6 mean_story_norm, 7 max_story_norm, 8 std_story_norm,
# 9 mean_dist_norm, 10 min_dist_norm, 11 story_valid_ratio
GLOBAL_BASE_DIM = 12
DEFAULT_GLOBAL_SCALES_M = [50.0, 100.0, 150.0, 330.0, 500.0, 700.0]
GLOBAL_DIM = GLOBAL_BASE_DIM * len(DEFAULT_GLOBAL_SCALES_M)

# Edge types used by edge-type-aware graph attention.
# 0 padding, 1 self,
# 2/3/4 kNN near/mid/far,
# 5/6/7 radius near/mid/far,
# 8/9/10 hybrid near/mid/far.
NUM_EDGE_TYPES = 11
...
class BuildingGraphProviderV8:
    """
    V8 graph provider.

    Key differences from V6/V7:
    1) Core-aware multi-scale global statistics: 50/100/150/330/500/700 m.
    2) Ring-balanced salient node selection to preserve inner/middle/outer LCZ morphology.
    3) V7 node/edge representation is retained: local density, nearest-neighbor distance,
       salient score, and distance-binned edge types.
    4) Reliability-aware interaction fusion: graph contribution is modulated by global building reliability.
    """
    def __init__(self,
                 shp_path: Path,
                 gt_path: Path,
                 ...
                 num_sectors: int = 8,
                 ring_edges_m: Optional[list] = None,
                 ring_node_ratios: Optional[list] = None):
        ...
        # V8: ring-balanced node budget. Default = inner/middle/outer around target LCZ cell.
        self.ring_edges_m = [float(v) for v in (ring_edges_m if ring_edges_m is not None else [150.0, 330.0, self.context_m])]
        if self.ring_edges_m[-1] < self.context_m:
            self.ring_edges_m.append(float(self.context_m))
        self.ring_edges_m = sorted(self.ring_edges_m)
        self.ring_node_ratios = [float(v) for v in (ring_node_ratios if ring_node_ratios is not None else [0.50, 0.30, 0.20])]
        if len(self.ring_node_ratios) != len(self.ring_edges_m):
            self.ring_node_ratios = [1.0 / len(self.ring_edges_m)] * len(self.ring_edges_m)
        s = max(sum(self.ring_node_ratios), 1e-6)
        self.ring_node_ratios = [v / s for v in self.ring_node_ratios]
        ...
        print(f"Ring-balanced selection: edges={self.ring_edges_m}, ratios={self.ring_node_ratios}")
    ...
    def select_salient_nodes(self, idx, dx, dy, dist):
        """
        V8 ring-balanced salient node selection.

        V6 selected near/large/high/sector buildings globally. V8 preserves that saliency,
        but allocates node budget across distance rings so that the graph keeps:
        - core morphology around the target LCZ cell,
        - middle-range urban texture,
        - outer LCZ context.
        """
        n = len(idx)
        if n <= self.max_nodes:
        ...
        area_norm_all = np.clip(np.log1p(self.area[idx]) / self.area_scale, 0.0, 1.0)
        story_norm_all = np.clip(self.story[idx] / self.story_scale, 0.0, 1.0)
        dist_score_all = np.clip(1.0 - dist / max(self.search_radius, 1e-6), 0.0, 1.0)

        # local density proxy among all candidates, computed once for selection quality.
        xy = np.stack([dx, dy], axis=1).astype(np.float32)
        local_density_score = np.zeros((n,), dtype=np.float32)
        if n > 1:
            dmat = np.sqrt(((xy[:, None, :] - xy[None, :, :]) ** 2).sum(axis=2))
            local_density_score = np.clip(((dmat <= 80.0) & (dmat > 0)).sum(axis=1).astype(np.float32) / 20.0, 0.0, 1.0)

        saliency = (
            0.35 * dist_score_all
            + 0.25 * area_norm_all
            + 0.25 * story_norm_all
            + 0.15 * local_density_score
        )
        ...
        # Ring-balanced allocation.
        prev_edge = 0.0
        remaining_total = self.max_nodes
        for ring_i, edge in enumerate(self.ring_edges_m):
            if ring_i == len(self.ring_edges_m) - 1:
                budget = remaining_total
            else:
                budget = max(1, int(round(self.max_nodes * self.ring_node_ratios[ring_i])))
                budget = min(budget, remaining_total)
            ring_mask = (dist > prev_edge) & (dist <= edge)
            cand = np.where(ring_mask)[0]
            if len(cand) > 0 and budget > 0:
                order = cand[np.argsort(-saliency[cand])[:budget]]
                add_indices(order)
                remaining_total = self.max_nodes - len(selected)
            prev_edge = edge
            if len(selected) >= self.max_nodes:
                break

        # Sector-balanced fill for spatial coverage if some rings were sparse.
        if len(selected) < self.max_nodes:
            ...
            for s in range(self.num_sectors):
                ...
                if len(cand) == 0:
                    continue
                order = cand[np.argsort(-saliency[cand])[:per_sector]]
                add_indices(order)
                if len(selected) >= self.max_nodes:
                    break

        # Final fill by saliency.
        if len(selected) < self.max_nodes:
            add_indices(np.argsort(-saliency))
    ...
    def get_graph(self, row50: int, col50: int):
        ...
        local_count_density = np.zeros_like(area_norm, dtype=np.float32)
        local_area_density = np.zeros_like(area_norm, dtype=np.float32)
        local_story = np.zeros_like(area_norm, dtype=np.float32)
        nearest_dist_norm = np.ones_like(area_norm, dtype=np.float32)
        if len(idx) > 1:
            dmat = np.sqrt(((xy_rel[:, None, :] - xy_rel[None, :, :]) ** 2).sum(axis=2))
            neigh = (dmat <= 80.0) & (dmat > 0)
            local_count = neigh.sum(axis=1).astype(np.float32)
            local_count_density = np.clip(local_count / 20.0, 0.0, 1.0)
            local_area_sum = (neigh.astype(np.float32) * area[None, :]).sum(axis=1)
            local_area_density = np.clip(local_area_sum / (math.pi * 80.0 * 80.0), 0.0, 1.0)
            d_no_self = dmat + np.eye(len(idx), dtype=np.float32) * 1e6
            nearest_dist_norm = np.clip(d_no_self.min(axis=1) / max(self.radius_m, 1e-6), 0.0, 1.0).astype(np.float32)
            for i in range(len(idx)):
                vals = story[neigh[i] & (story > 0)]
                local_story[i] = float(np.clip(vals.mean() / self.story_scale, 0.0, 1.0)) if len(vals) else 0.0

        salient_score = (
            0.40 * np.clip(1.0 - dist / max(self.search_radius, 1e-6), 0.0, 1.0)
            + 0.35 * area_norm
            + 0.25 * story_norm
        ).astype(np.float32)
        ...
        node = np.stack([
            ...
            local_count_density, local_area_density, local_story,
            nearest_dist_norm, salient_score, has_building,
        ], axis=1).astype(np.float32)

        adj, etype = self.make_adj_and_edge_type(xy_rel)
    ...
    def make_adj_and_edge_type(self, xy_rel: np.ndarray):
        ...
        def dist_bin_code(dist_val: float, base_code: int) -> int:
            # base_code should be 2 for kNN, 5 for radius, 8 for hybrid.
            if dist_val <= self.radius_m * 0.5:
                return base_code
            if dist_val <= self.radius_m:
                return base_code + 1
            return base_code + 2
        ...
        if self.edge_mode in ["knn", "hybrid"]:
            ...
            for i in range(n):
                ...
                for j in nn_idx:
                    w = float(np.exp(-d[i, j] / sigma))
                    code = dist_bin_code(float(d[i, j]), 2)
                    if w > adj[i, j]:
                        adj[i, j] = adj[j, i] = w
                    etype[i, j] = etype[j, i] = max(etype[i, j], code)

        if self.edge_mode in ["radius", "hybrid"]:
            radius_mask = (d <= self.radius_m) & (d > 0)
            src, dst = np.where(radius_mask)
            for i, j in zip(src, dst):
                if i == j:
                    continue
                w = float(np.exp(-d[i, j] / sigma))
                if self.edge_mode == "hybrid" and adj[i, j] > 0:
                    code = dist_bin_code(float(d[i, j]), 8)
                else:
                    code = dist_bin_code(float(d[i, j]), 5)
                if w > adj[i, j]:
                    adj[i, j] = w
                etype[i, j] = max(etype[i, j], code)
    ...
    def make_multiscale_global_features(self, idx_all, dx_all, dy_all, dist_all):
        ...
        for scale_m in self.global_scales_m:
            ...
            area = self.area[idx]
            story = self.story[idx]
            feats.append(self.make_global_features_for_scale(area, story, dist, scale_m))
        return np.concatenate(feats, axis=0).astype(np.float32)

    def make_global_features_for_scale(self, area: np.ndarray, story: np.ndarray, dist: np.ndarray, scale_m: float):
        n = len(area)
        if n == 0:
        ...
        built_ratio = np.clip(float(area.sum()) / context_area_m2, 0.0, 1.0)
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: BuildingGraphProviderV8, cache_path: Path, split_name: str):
...
class CachedLCZGraphDatasetV8(Dataset):
...
class GraphAttentionBlockV8(nn.Module):
...
class ScaleAwareGlobalEncoder(nn.Module):
    def __init__(self, base_dim: int = GLOBAL_BASE_DIM, num_scales: int = 3, hidden_dim: int = 96, dropout: float = 0.1):
        super().__init__()
        self.base_dim = base_dim
        self.num_scales = num_scales
        self.scale_mlp = nn.Sequential(
            nn.Linear(base_dim, hidden_dim), nn.GELU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout)
        )
        self.scale_embed = nn.Parameter(torch.zeros(1, num_scales, hidden_dim))
        self.score = nn.Sequential(nn.Linear(hidden_dim, hidden_dim // 2), nn.Tanh(), nn.Linear(hidden_dim // 2, 1))

    def forward(self, glob):
        B = glob.shape[0]
        g = glob.view(B, self.num_scales, self.base_dim)
        h = self.scale_mlp(g) + self.scale_embed
        w = torch.softmax(self.score(h).squeeze(-1), dim=1).unsqueeze(-1)
        return (h * w).sum(dim=1)


class BuildingGraphEncoderV8(nn.Module):
    def __init__(self, in_dim: int = NODE_DIM, global_dim: int = GLOBAL_DIM, hidden_dim: int = 96, out_dim: int = 192, num_layers: int = 3, heads: int = 4, dropout: float = 0.1):
        super().__init__()
        if global_dim % GLOBAL_BASE_DIM != 0:
            raise ValueError(f"global_dim {global_dim} must be divisible by GLOBAL_BASE_DIM {GLOBAL_BASE_DIM}")
        self.input_mlp = nn.Sequential(nn.Linear(in_dim, hidden_dim), nn.GELU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout))
        self.layers = nn.ModuleList([GraphAttentionBlockV8(hidden_dim, heads=heads, dropout=dropout) for _ in range(num_layers)])
        self.attn_pool = AttentionPool(hidden_dim)
        self.global_encoder = ScaleAwareGlobalEncoder(
            base_dim=GLOBAL_BASE_DIM,
            num_scales=global_dim // GLOBAL_BASE_DIM,
...
class RSGraphLCZModelV8(nn.Module):
    def __init__(self, rs_channels: int, num_classes: int, patch_size: int = 33, graph_hidden: int = 96, graph_out: int = 192, graph_layers: int = 3, graph_heads: int = 4, dropout: float = 0.35):
        ...
        self.graph_encoder = BuildingGraphEncoderV8(in_dim=NODE_DIM, global_dim=GLOBAL_DIM, hidden_dim=graph_hidden, out_dim=graph_out, num_layers=graph_layers, heads=graph_heads, dropout=dropout * 0.4)
        self.graph_reliability = nn.Sequential(
        ...
        self.graph_to_rs = nn.Sequential(
            nn.LayerNorm(graph_out),
            nn.Linear(graph_out, 256),
            nn.GELU(),
        )
        fusion_dim = 256 + graph_out + 256
        self.fusion = nn.Sequential(
            nn.LayerNorm(fusion_dim),
            nn.Dropout(dropout),
            nn.Linear(fusion_dim, 384), nn.GELU(),
    ...
    def forward(self, rs, node, adj, edge_type, mask, glob):
        ...
        # V8: reliability-aware fusion. If building information is sparse or unreliable,
        # graph contribution is softly reduced; otherwise it is amplified.
        reliability = self.graph_reliability(glob)  # [B, 1], range 0~1
        graph_feat_w = graph_feat * (0.5 + reliability)
        graph_rs = self.graph_to_rs(graph_feat_w)
        interaction = rs_feat * graph_rs
        return self.fusion(torch.cat([rs_feat, graph_feat_w, interaction], dim=1))
...
def train(args):
    ...
    tag = f"v8_ctx{int(args.graph_context_m if args.graph_context_m else args.patch_size*10)}_gs{'-'.join(str(int(x)) for x in args.global_scales_m)}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}"
    cache_paths = {
    ...
    if need_cache:
        ...
        graph_provider = BuildingGraphProviderV8(
            ...
            ring_edges_m=args.ring_edges_m,
            ring_node_ratios=args.ring_node_ratios,
        )
    ...
    train_ds = CachedLCZGraphDatasetV8(rs_stack, cache_paths["train"], args.patch_size)
    val_ds = CachedLCZGraphDatasetV8(rs_stack, cache_paths["val"], args.patch_size)
    test_ds = CachedLCZGraphDatasetV8(rs_stack, cache_paths["test"], args.patch_size)
    print(f"Dataset: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v8)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v8)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v8)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    model = RSGraphLCZModelV8(
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V8",
        ...
        "ring_edges_m": [float(v) for v in args.ring_edges_m],
        "ring_node_ratios": [float(v) for v in args.ring_node_ratios],
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v8")
    ...
    p.add_argument("--ring_edges_m", type=float, nargs="+", default=[150.0, 330.0, 500.0])
    p.add_argument("--ring_node_ratios", type=float, nargs="+", default=[0.50, 0.30, 0.20])
