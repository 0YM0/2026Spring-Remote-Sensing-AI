#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V10

V10 proposal: keep the V7 graph construction/encoder exactly as the strong baseline,
but replace plain late concat with a safer graph-conditioned FiLM/gating fusion.

Why this version after V9:
- V9 cross-attention was more expressive but underperformed V7 in patch-level LCZ classification.
- V10 therefore keeps V7 late-fusion stability and adds only a weak, zero-initialized
  graph-conditioned modulation of the RS feature vector.
- At initialization, the RS adapter is identity-like, so training starts close to V7.

Recommended first run:
CUDA_VISIBLE_DEVICES=0 python train/train_lcz_graph_encoder_v10.py \
  ...
  --exp_name cnn_rs_building_graph_encoder_v10_film \
  ...
  --film_hidden 128 --film_residual_init 0.05 \
  --batch_size 96 --epochs 180 --patience 35 --lr 5e-4 --dropout 0.35 \
  --class_weight --monitor oa
"""
...
class GraphConditionedFiLMAdapterV10(nn.Module):
    """
    Conservative graph-conditioned feature modulation for LCZ patch classification.

    Input:
    - rs_feat:    [B, 256] CNN patch descriptor
    - graph_feat: [B, graph_out] V7 graph descriptor

    Output:
    - rs_mod: graph-conditioned RS descriptor

    Design choice:
    - The last FiLM layer is zero-initialized, so gamma=0 and beta=0 at start.
    - Therefore rs_mod == rs_feat at initialization, preserving V7-like behavior.
    - A small learnable residual scale controls how strongly graph context can alter RS features.
    """
    def __init__(self, rs_dim: int = 256, graph_dim: int = 192, hidden_dim: int = 128, dropout: float = 0.1, residual_init: float = 0.05):
        super().__init__()
        self.rs_norm = nn.LayerNorm(rs_dim)
        self.graph_norm = nn.LayerNorm(graph_dim)
        self.film = nn.Sequential(
            nn.Linear(graph_dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, rs_dim * 2),
        )
        # Zero-init makes the adapter identity-like at initialization.
    ...
    def forward(self, rs_feat, graph_feat):
        rs_base = rs_feat
        rs_norm = self.rs_norm(rs_feat)
        graph_norm = self.graph_norm(graph_feat)
        gamma, beta = self.film(graph_norm).chunk(2, dim=-1)
        # tanh bounds the modulation and avoids unstable feature explosion.
        gamma = torch.tanh(gamma)
        beta = torch.tanh(beta)
        delta = rs_norm * gamma + beta
        return rs_base + self.residual_scale * delta


class GraphAwareFusionHeadV10(nn.Module):
    """
    V7-compatible late fusion head with graph-conditioned RS modulation.

    It still concatenates [rs_mod, graph_feat], which was the best-performing V7 pattern,
    but rs_mod is lightly conditioned by graph morphology before concatenation.
    """
    def __init__(self, rs_dim: int = 256, graph_dim: int = 192, num_classes: int = 11, film_hidden: int = 128, dropout: float = 0.35, film_residual_init: float = 0.05):
        super().__init__()
        self.adapter = GraphConditionedFiLMAdapterV10(
            rs_dim=rs_dim,
            graph_dim=graph_dim,
            hidden_dim=film_hidden,
            dropout=dropout * 0.4,
            residual_init=film_residual_init,
        )
    ...
    def forward(self, rs_feat, graph_feat):
        rs_mod = self.adapter(rs_feat, graph_feat)
        return self.classifier(torch.cat([rs_mod, graph_feat], dim=1))


class RSGraphLCZModelV10(nn.Module):
    def __init__(self,
                 rs_channels: int,
                 num_classes: int,
                 patch_size: int = 33,
                 ...
                 graph_heads: int = 4,
                 film_hidden: int = 128,
                 film_residual_init: float = 0.05,
                 dropout: float = 0.35):
        super().__init__()
        # Keep V7 feature dimensions and encoder capacity.
        self.rs_encoder = ConvEncoder(rs_channels, patch_size, out_dim=256, base_channels=32, dropout=dropout * 0.3)
        self.graph_encoder = BuildingGraphEncoderV7(
            in_dim=NODE_DIM,
        ...
        self.fusion = GraphAwareFusionHeadV10(
            rs_dim=256,
            graph_dim=graph_out,
            num_classes=num_classes,
            film_hidden=film_hidden,
            dropout=dropout,
            film_residual_init=film_residual_init,
        )
...
def train(args):
    ...
    model = RSGraphLCZModelV10(
        ...
        film_residual_init=args.film_residual_init,
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V10",
        ...
        "film_residual_init": float(args.film_residual_init),
        "loss": args.loss,
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v10_film")
    ...
    p.add_argument("--film_residual_init", type=float, default=0.05)
