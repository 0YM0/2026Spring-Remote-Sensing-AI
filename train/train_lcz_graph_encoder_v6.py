#!/usr/bin/env python3
"""
CNN_RS_BuildingGraphEncoder_V6

V6 proposal: Salient-node + multi-scale statistics + edge-type-aware attention.
...
CUDA_VISIBLE_DEVICES=2 python train/train_lcz_graph_encoder_v6.py \
  ...
  --exp_name graph_v6_salient_multiscale_edgeaware \
  --patch_size 33 \
  --graph_context_m 500 \
  ...
  --class_weight \
  --monitor oa \
  --exp_name graph_v6_salient_multiscale_edgeaware_oa
...
# 13 local_density_80m, 14 local_story_80m, 15 has_building
NODE_DIM = 16
...
# 0 none/padding, 1 self, 2 knn, 3 radius, 4 knn+radius
NUM_EDGE_TYPES = 5
...
class BuildingGraphProviderV6:
    ...
    3) Edge type matrix: distinguishes self, kNN, radius, and hybrid edges.
    """
    def __init__(self,
                 shp_path: Path,
                 gt_path: Path,
        ...
        if len(self.global_scales_m) != len(DEFAULT_GLOBAL_SCALES_M):
            raise ValueError(f"V6 expects exactly {len(DEFAULT_GLOBAL_SCALES_M)} global scales. Got {self.global_scales_m}")
    ...
    def get_graph(self, row50: int, col50: int):
        ...
        node = np.stack([
            ...
            local_density, local_story, has_building,
    ...
    def make_adj_and_edge_type(self, xy_rel: np.ndarray):
        ...
        if self.edge_mode in ["radius", "hybrid"]:
            ...
            # If an edge already exists as kNN, mark as hybrid type 4.
...
def build_graph_cache_for_split(sample_npz: Path, graph_provider: BuildingGraphProviderV6, cache_path: Path, split_name: str):
...
class CachedLCZGraphDatasetV6(Dataset):
...
class GraphAttentionBlockV6(nn.Module):
...
class BuildingGraphEncoderV6(nn.Module):
    def __init__(self, in_dim: int = NODE_DIM, global_dim: int = GLOBAL_DIM, hidden_dim: int = 96, out_dim: int = 192, num_layers: int = 3, heads: int = 4, dropout: float = 0.1):
        ...
        self.layers = nn.ModuleList([GraphAttentionBlockV6(hidden_dim, heads=heads, dropout=dropout) for _ in range(num_layers)])
...
class RSGraphLCZModelV6(nn.Module):
    def __init__(self, rs_channels: int, num_classes: int, patch_size: int = 33, graph_hidden: int = 96, graph_out: int = 192, graph_layers: int = 3, graph_heads: int = 4, dropout: float = 0.35):
        ...
        self.graph_encoder = BuildingGraphEncoderV6(in_dim=NODE_DIM, global_dim=GLOBAL_DIM, hidden_dim=graph_hidden, out_dim=graph_out, num_layers=graph_layers, heads=graph_heads, dropout=dropout * 0.4)
...
def train(args):
    ...
    tag = f"v6_ctx{int(args.graph_context_m if args.graph_context_m else args.patch_size*10)}_gs{'-'.join(str(int(x)) for x in args.global_scales_m)}_{args.edge_mode}_k{args.knn}_r{int(args.radius_m)}_n{args.max_nodes}"
    cache_paths = {
    ...
    if need_cache:
        ...
        graph_provider = BuildingGraphProviderV6(
    ...
    train_ds = CachedLCZGraphDatasetV6(rs_stack, cache_paths["train"], args.patch_size)
    val_ds = CachedLCZGraphDatasetV6(rs_stack, cache_paths["val"], args.patch_size)
    test_ds = CachedLCZGraphDatasetV6(rs_stack, cache_paths["test"], args.patch_size)
    print(f"Dataset: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v6)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v6)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=graph_collate_v6)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    model = RSGraphLCZModelV6(
    ...
    summary = {
        "model": "CNN_RS_BuildingGraphEncoder_V6",
...
def parse_args():
    ...
    p.add_argument("--exp_name", type=str, default="cnn_rs_building_graph_encoder_v6")
